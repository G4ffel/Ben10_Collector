document.addEventListener('DOMContentLoaded', () => {
  ...
  const setViewMode = (mode) => {
    if (mode === 'table') {
      ...
      if (alienGridView) alienGridView.style.display = 'flex';
  ...
  const filterAliens = () => {
    ...
    alienItems.forEach(item => {
      ...
      const subcat = (item.getAttribute('data-subcategoria') || '').toLowerCase();
    ...
    // Actualizar visibilidad de grupos de subcategorías en Grid
    document.querySelectorAll('.subcategory-group-wrapper').forEach(wrapper => {
      const visibleItems = wrapper.querySelectorAll('.alien-item:not([style*="display: none"])');
      wrapper.style.display = visibleItems.length > 0 ? 'flex' : 'none';
    });

    // Actualizar visibilidad de encabezados de subcategorías en Tabla
    const tableHeaders = document.querySelectorAll('.subcategory-table-header');
    tableHeaders.forEach(header => {
      let nextElem = header.nextElementSibling;
      let hasVisibleChild = false;
      while (nextElem && !nextElem.classList.contains('subcategory-table-header')) {
        if (nextElem.classList.contains('alien-item') && nextElem.style.display !== 'none') {
          hasVisibleChild = true;
          break;
        }
        nextElem = nextElem.nextElementSibling;
      }
      header.style.display = hasVisibleChild ? 'table-row' : 'none';
    });
  };
  ...
  const openZoomModal = (nombre, serie, imagenSrc) => {
    ...
    // Determinar estilo de serie según la solicitud:
    // - Todos los Aliens (Ben 10, Alien Force, Omniverse): Verde Omnitrix (var(--green-primary))
    // - Villanos: Rojo (#ff3333)
    // - Personajes: Otro tipo de verde (Verde Neón Menta: #00ffaa)
    ...
    if (serieLower.includes('villano')) {
      color = '#ff3333';
      shadow = 'rgba(255, 51, 51, 0.4)';
      badgeClass = 'serie-villanos';
    } else if (serieLower.includes('personaje')) {
      color = '#00ffaa';
      ...
      if (serie.includes('Alien Force')) {
        badgeClass = 'serie-af';
      } else if (serie.includes('Omniverse')) {
        badgeClass = 'serie-ov';
      } else {
        badgeClass = 'serie-classic';
      }
    }
